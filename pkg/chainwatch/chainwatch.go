...
//
// Every entry arriving here is signed by the seat that took it, so this peer
// rebuilds the chain itself and checks each one rather than accepting anybody's
// account of the game. There is no party at the table holding a version of events
// that this could be checked against, which is the point: the chain each peer
// holds is the only account there is, and they are comparable.
//
// What that buys is not the detection of a lie by itself; it is that a
// disagreement becomes evidence. Two peers whose chains diverge hold signed
// messages saying so, verifiable by anybody who was not there, rather than a
// complaint about each other.
...
// New starts a verifier for a match.
//
...
func New(matchID string, roster map[uint32][]byte) (*ChainWatch, error) {
