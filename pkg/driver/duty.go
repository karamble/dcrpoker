...
// The duty vocabulary lives in the gaming schema; aliases keep every caller compiling.
type (
	Duty     = schema.Duty
	DutyKind = schema.DutyKind
)

const (
	DutyCardKey    = schema.DutyCardKey
	DutyShuffle    = schema.DutyShuffle
	DutyShare      = schema.DutyShare
	DutyAction     = schema.DutyAction
	DutyCheckpoint = schema.DutyCheckpoint
	DutyReveal     = schema.DutyReveal
)
